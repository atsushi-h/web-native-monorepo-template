...
### 規模別の概要（最小限の判定基準）
- **小規模**: 1-2ファイル、単一機能の修正 → **PLAN/TASK必須**
- **中規模**: 3-5ファイル、複数コンポーネントに跨る → **PLAN/TASK必須**
- **大規模**: 6ファイル以上、アーキテクチャレベルの変更 → **PLAN/TASK必須**

※現在の体系では、すべての規模でPLAN（作業計画書）とTASK（タスク分解）が必須
...
### 必要なドキュメント
- PLAN（作業計画書）: [必須]（理由: 全規模で必須 - 要件定義、技術設計、実装計画を統合）
- TASK（タスク分解）: [必須]（理由: 全規模で必須 - 実行可能な粒度でのタスク分解）

### 判定の根拠
- ファイル数: [数値]（規模: [small/medium/large]）
- PLAN/TASK必要性: 全規模で必須（統一フローによる品質保証）
